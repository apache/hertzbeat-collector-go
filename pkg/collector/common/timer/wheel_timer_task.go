...
type MetricsTaskDispatcher interface {
	DispatchMetricsTask(timeout *jobtypes.Timeout) error
}
...
func NewWheelTimerTask(job *jobtypes.Job, dispatcher MetricsTaskDispatcher, logger logger.Logger) *WheelTimerTask {
	task := &WheelTimerTask{
...
func (wtt *WheelTimerTask) initJobMetrics() {
	...
	configMap := make(map[string]*jobtypes.Configmap)
	...
	processedMetrics := make([]jobtypes.Metrics, 0, len(wtt.job.Metrics))
	...
	for _, metric := range wtt.job.Metrics {
		...
		var processedMetric jobtypes.Metrics
...
func (wtt *WheelTimerTask) replacePlaceholders(jsonStr string, configMap map[string]*jobtypes.Configmap) string {
...
func (wtt *WheelTimerTask) replaceFieldsForPushStyleMonitor(metric *jobtypes.Metrics, configMap map[string]*jobtypes.Configmap) {
