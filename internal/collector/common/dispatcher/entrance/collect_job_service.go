...
type CollectJobService struct {
	...
	responseListeners map[int64]chan []job.CollectRepMetricsData
...
func NewCollectJobService(
	...
	return &CollectJobService{
		...
		responseListeners: make(map[int64]chan []job.CollectRepMetricsData),
	}
}
...
func (cjs *CollectJobService) CollectSyncJobData(job *job.Job) ([]job.CollectRepMetricsData, error) {
	responseChan := make(chan []job.CollectRepMetricsData, 1)
	...
	case <-time.After(120 * time.Second):
		cjs.logger.Info("sync task runs for 120 seconds with no response, returning empty result",
			"jobId", job.MonitorID)
		return []job.CollectRepMetricsData{}, nil
	}
}

// CollectSyncOneTimeJobData executes a one-time collection task and sends the response.
func (cjs *CollectJobService) CollectSyncOneTimeJobData(job *job.Job) error {
...
func (cjs *CollectJobService) AddAsyncCollectJob(job *job.Job) error {
...
func (cjs *CollectJobService) DispatchCollectData(timeout *job.Timeout, metrics *job.Metrics, metricsData []*worker.MetricsData) error {
	// Convert worker.MetricsData to jobtypes.CollectRepMetricsData
	convertedData := make([]job.CollectRepMetricsData, 0, len(metricsData))
	for _, data := range metricsData {
		convertedData = append(convertedData, job.CollectRepMetricsData{
			ID:        data.ID,
...
func (cjs *CollectJobService) SendAsyncCollectData(metricsData job.CollectRepMetricsData) error {
	if cjs.networkClient == nil {
		return fmt.Errorf("network client is not set")
	}

	// Serialize metrics data
	data, err := cjs.serializeMetricsData([]job.CollectRepMetricsData{metricsData})
	if err != nil {
...
func (cjs *CollectJobService) SendAsyncServiceDiscoveryData(metricsData job.CollectRepMetricsData) error {
	if cjs.networkClient == nil {
		return fmt.Errorf("network client is not set")
	}
...
func (cjs *CollectJobService) ResponseSyncJobData(jobID int64, metricsData []job.CollectRepMetricsData) {
...
func (cjs *CollectJobService) serializeMetricsData(metricsData []job.CollectRepMetricsData) ([]byte, error) {
...
func convertStringArraysToValueRows(values [][]string) []job.ValueRow {
	valueRows := make([]job.ValueRow, len(values))
	for i, row := range values {
		valueRows[i] = job.ValueRow{Columns: row}
	}
	return valueRows
}
