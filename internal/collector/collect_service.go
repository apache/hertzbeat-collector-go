...
func (cs *CollectService) Collect(metrics *job.Metrics) *job.CollectRepMetricsData {
	if metrics == nil {
...
func (cs *CollectService) DispatchMetricsTask(timeout *job.Timeout) error {
	if timeout == nil {
...
func (cs *CollectService) createAndSubmitTask(metrics *job.Metrics, timeout *job.Timeout, job *job.Job) error {
	// Validate metrics
	if metrics == nil {
...
func (cs *CollectService) calculateTaskPriority(metrics *job.Metrics, job *job.Job) int {
...
func (cs *CollectService) collectWithPriority(metrics *job.Metrics, timeout *job.Timeout, priority int) *job.CollectRepMetricsData {
	// Create channel for result
	resultChan := make(chan *job.CollectRepMetricsData, 1)
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				...
				resultChan <- &job.CollectRepMetricsData{
					Code: 500,
